...
WeixinApi.ready(function(Api) {
	var wxData = window.shareData;
	var wxCallbacks = {
		favorite : false,
		async : true,
		ready : function() {
			this.dataLoaded(window.shareData);
		},
		cancel : function(resp) {
			if(window.shareData.cancel) window.shareData.cancel(resp)
		},
		fail : function(resp) {
			if(window.shareData.fail) window.shareData.fail(resp)
		},
		confirm : function(resp) {
			if(window.shareData.success) window.shareData.success(resp)
		},
		all : function(resp,shareTo) {
			if(window.shareData.trigger) window.shareData.trigger(resp)
		}
	};
	Api.shareToFriend(wxData, wxCallbacks);
	Api.shareToTimeline(wxData, wxCallbacks);
