...
var adaptUILayout = (function(){
	...
	var getViewport = function(uiWidth) {
	...
	var adapt = function(uiWidth){
		...
		viewport.content = getViewport(uiWidth);
		if (head.length > 0) {
			head = head[head.length - 1];
			var history = document.getElementById('viewport');
			history && head.removeChild(history);
			head.appendChild(viewport);
		}
	};

	var load = function(uiWidth){
		var viewport = getViewport(uiWidth);
